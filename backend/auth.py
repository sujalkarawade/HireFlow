...
def get_current_candidate(current_user: models.User = Depends(get_current_user)):
    if current_user.role != "candidate":
        raise HTTPException(status_code=403, detail="Not enough permissions. Role must be candidate.")
    return current_user

def get_current_recruiter(current_user: models.User = Depends(get_current_user)):
    if current_user.role != "recruiter":
        raise HTTPException(status_code=403, detail="Not enough permissions. Role must be recruiter.")
    return current_user
